...
<template>
    <q-page>
        <div class="q-pa-md">
            <q-form @submit="handleRetirada" v-if="produtosOptions.length > 0">
                    <q-select
                        v-model="Id_produto"
                    ...
                    <q-input
                        ...
                        :disable="!Id_produto"
                    />
                    <q-btn label="Registrar Retirada" color="primary" type="submit" />
...
export default{
    ...
    try {
        ...
        await axios.post('http://localhost:3000/RetiraProduto', payload);

        Id_produto.value = null;
        quantidade.value = null;

        $q.notify({
            message: 'Retirada do produto realizada!',
            color: 'green',
            position: 'top',
        });
    } catch (error) {
        ...
        $q.notify({
            message: 'Não foi possível registrar a Retirada do produto!',
            color: 'red',
            position: 'top',
        });
    }
